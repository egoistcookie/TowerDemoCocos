        ...
        try {
            if (sys.platform === sys.Platform.WECHAT_GAME && typeof window !== 'undefined') {
                ...
                if (wx && typeof wx.getLaunchOptionsSync === 'function') {
                    ...
                    if (wxLaunchOptions) {
                        // 微信小程序场景值列表（完整文档：https://developers.weixin.qq.com/miniprogram/dev/reference/scene-list.html）
                        // 常用场景值：
                        //   1001 - 发现栏小程序主入口，顶部搜索框的搜索结果页
                        //   1005 - 顶部搜索框的搜索结果页
                        //   1006 - 发现栏小程序主入口
                        //   1007 - 单人聊天会话中的消息卡片
                        //   1008 - 群聊会话中的消息卡片
                        ...
                        // 场景值判断来源渠道
                        const adScenes = [1035, 1044, 1036, 1037]; // 1035 小程序广告，1044 朋友圈广告，1036 APP 分享卡片，1037 小程序推广计划
                        if (adScenes.indexOf(wxLaunchOptions.scene) >= 0) {
                            ...
                            channel = 'launcher'; // 顶部搜索/历史记录
                        } else if (wxLaunchOptions.scene === 1043) {
                            channel = 'public_account'; // 公众号模板消息
                        } else if (wxLaunchOptions.scene === 1089) {
                            channel = 'nearby'; // 附近的小程序
                        } else {
                            channel = 'scene_' + wxLaunchOptions.scene; // 其他场景
                        }
