...
export class ElfSwordsman extends Component {
    @property
    maxHealth: number = 50;
    ...
    // 音效相关属性
    @property(AudioClip)
    attackSound: AudioClip = null!; // 近战攻击时的音效
    ...
    initAttackAnimation() {
        ...
        if (this.attackAnimationFrames && this.attackAnimationFrames.length > 0) {
            ...
            if (validFrames.length < this.attackAnimationFrames.length) {
                console.warn(`ElfSwordsman: Warning - ${this.attackAnimationFrames.length - validFrames.length} frames are null or invalid!`);
            }
            return;
        }
        ...
        if (this.attackAnimationTexture) {
            ...
            console.warn('ElfSwordsman: No attack animation frames or texture set. Attack animation will not play.');
        }
    }
    ...
    loadFramesFromTexture() {
        ...
        console.warn('ElfSwordsman: Auto-loading frames from texture is not fully supported. Please set attackAnimationFrames array in editor, or use SpriteAtlas.');
    }
    ...
    createHealthBar() {
        ...
        this.healthBarNode.setPosition(0, 30, 0); // 在精灵剑士上方
        // 确保血条初始缩放为正数（正常朝向）
    ...
    update(deltaTime: number) {
        ...
        if (this.currentTarget && this.currentTarget.isValid && this.currentTarget.active) {
            ...
            const detectionRange = 400; // 和弓箭手一样的索敌范围(400像素)
    ...
    checkCollisionAtPosition(position: Vec3): boolean {
        ...
        if (scene) {
            if (!towersNode) towersNode = findNodeRecursive(scene, 'Towers');
            if (!huntersNode) huntersNode = findNodeRecursive(scene, 'Hunters');
            if (!swordsmenNode) swordsmenNode = findNodeRecursive(scene, 'ElfSwordsmans');
        }
        ...
        if (huntersNode) {
            ...
            for (const hunter of hunters) {
                if (hunter && hunter.isValid && hunter.active && hunter !== this.node) {
                    ...
                    const otherHunterScript = hunter.getComponent('Hunter') as any;
        ...
        if (swordsmenNode) {
            ...
            for (const swordsman of swordsmen) {
                if (swordsman && swordsman.isValid && swordsman.active && swordsman !== this.node) {
                    ...
                    const otherSwordsmanScript = swordsman.getComponent('ElfSwordsman') as any;
    ...
    calculatePushAwayDirection(currentPos: Vec3): Vec3 {
        ...
        // 检查其他友方单位（弓箭手、女猎手、精灵剑士）
        const towersNode = find('Towers');
        const huntersNode = find('Hunters');
        const swordsmenNode = find('ElfSwordsmans');
        ...
        if (swordsmenNode) {
            ...
            for (const swordsman of swordsmen) {
                if (swordsman && swordsman.isValid && swordsman.active && swordsman !== this.node) {
                    ...
                    const distance = Vec3.distance(currentPos, swordsmanPos);
                    const otherSwordsmanScript = swordsman.getComponent('ElfSwordsman') as any;
                    const otherRadius = otherSwordsmanScript && otherSwordsmanScript.collisionRadius ? otherSwordsmanScript.collisionRadius : this.collisionRadius;
                    const minDistance = (this.collisionRadius + otherRadius) * 1.2;
                    if (distance < minDistance && distance > 0.1) {
                        const pushDir = new Vec3();
                        Vec3.subtract(pushDir, currentPos, swordsmanPos);
                        pushDir.normalize();
                        const strength = Math.max(2.0, (minDistance - distance) / minDistance * 3.0);
                        Vec3.scaleAndAdd(pushForce, pushForce, pushDir, strength);
                        maxPushStrength = Math.max(maxPushStrength, strength);
        ...
        if (swordsmenNode) {
            ...
            for (const swordsman of swordsmen) {
                if (swordsman && swordsman.isValid && swordsman.active && swordsman !== this.node) {
                    ...
                    if (distance < detectionRange && distance > 0.1) {
                        const avoidDir = new Vec3();
                        Vec3.subtract(avoidDir, currentPos, swordsmanPos);
                        avoidDir.normalize();
                        let strength = 1 - (distance / detectionRange);
                        if (distance < minDistance) {
                            strength = 3.0;
    ...
    setManualMoveTarget(event: EventTouch) {
        ...
        if (!cameraNode) {
            console.error('ElfSwordsman: Camera node not found!');
            return;
        }
        
        const camera = cameraNode.getComponent(Camera);
        if (!camera) {
            console.error('ElfSwordsman: Camera component not found!');
            return;
        }
    ...
    hasUnitAtMovePosition(position: Vec3, radius: number): boolean {
        ...
        let swordsmenNode = find('ElfSwordsmans');
        if (this.node.scene) {
            if (!towersNode) towersNode = findNodeRecursive(this.node.scene, 'Towers');
            if (!huntersNode) huntersNode = findNodeRecursive(this.node.scene, 'Hunters');
            if (!swordsmenNode) swordsmenNode = findNodeRecursive(this.node.scene, 'ElfSwordsmans');
        }
            ...
            for (const swordsman of swordsmen) {
                if (swordsman && swordsman.isValid && swordsman.active && swordsman !== this.node) {
                    ...
                    if (swordsmanScript && swordsmanScript.isAlive && swordsmanScript.isAlive()) {
                        const distance = Vec3.distance(position, swordsman.worldPosition);
