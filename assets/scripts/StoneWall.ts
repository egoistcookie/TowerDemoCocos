...
export class StoneWall extends Component {
    ...
    onStoneWallClick(event: EventTouch) {
        console.info('[StoneWall] onStoneWallClick - 节点点击事件触发, propagationStopped:', event.propagationStopped);
        ...
        console.info('[StoneWall] onStoneWallClick - 查找TowerBuilder, 节点找到:', !!towerBuilderNode, '组件找到:', !!towerBuilder, 'isDraggingBuilding:', towerBuilder?.isDraggingBuilding);
        
        // 检查是否正在长按检测（由TowerBuilder处理）
        // 注意：不要阻止事件传播，让TowerBuilder的onTouchEnd也能处理
        if (towerBuilder && (towerBuilder as any).isLongPressActive) {
            console.info('[StoneWall] onStoneWallClick - 检测到正在长按检测，不处理点击事件，让TowerBuilder处理');
        ...
        if (towerBuilder && towerBuilder.isDraggingBuilding) {
            console.info('[StoneWall] onStoneWallClick - 检测到正在拖拽建筑物，直接调用TowerBuilder.endDraggingBuilding处理');
    ...
    onMoveTouchEnd(event: EventTouch) {
        console.info('[StoneWall] onMoveTouchEnd - 触摸结束事件, isMoving:', this.isMoving, 'gridPanel存在:', !!this.gridPanel, 'propagationStopped:', event.propagationStopped);
        if (!this.isMoving || !this.gridPanel) {
            console.info('[StoneWall] onMoveTouchEnd - 不在移动状态或gridPanel不存在，直接返回');
            return;
        }
